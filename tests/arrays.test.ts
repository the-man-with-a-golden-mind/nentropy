...
describe('Array rendering', () => {
  it('renders initial array items into the DOM', () => {
    ...
    const realLis = Array.from(container.querySelectorAll('li')).filter(
  ...
  it('push adds a new element to the DOM', () => {
    ...
    `);
    data.list = ['a', 'b'];
    const before = container.querySelectorAll('li:not([en-mark$="#"])').length;
    data.list.push('c');
    const after = container.querySelectorAll('li:not([en-mark$="#"])').length;
